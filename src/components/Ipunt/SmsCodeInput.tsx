...
return (
    ...
    <input shrink-1 j-input-text type="text" placeholder={placeholder}
      ...
      {
        started 
        ? <button type='button' max-w="[calc(60%-8px)]" shrink-0 j-btn disabled text-gray>{count}后可以重新发送</button> 
        : <button type='button' max-w="[calc(60%-8px)]" shrink-0 j-btn onClick={_onClick}>发送验证码</button>
      }
  </div>)
} 